-- =================================================================
-- MIGRATION V26: ADD LATITUDE & LONGITUDE TO ORDERS & RPC UPDATE
-- =================================================================

BEGIN;

-- 1. Add columns to public.orders if not exists
DO $$
BEGIN
...
-- 2. Update the create_complete_order RPC to persist latitude and longitude from p_order
...
BEGIN
  ...
  -- 3. Create Order (Populates BOTH tenant_id AND company_id to align with RLS policies, now including latitude and longitude)
  DECLARE
    v_fee_col TEXT;
    v_status_col TEXT;
  BEGIN
    ...
    EXECUTE format(
      'INSERT INTO public.orders (tenant_id, company_id, customer_id, address_id, total, %I, order_type, notes, %I, discount, latitude, longitude)
    ...
    USING 
      ...
      (p_order->>'latitude')::NUMERIC, (p_order->>'longitude')::NUMERIC;
  END;
...
-- 3. Recreate public.pedidos view to include latitude and longitude
