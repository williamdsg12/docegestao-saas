-- =================================================================
-- MIGRATION V24: CUSTOMER PERSISTENCE AND VIEW ALIGNMENT
-- =================================================================

BEGIN;

-- 1. REWRITE create_complete_order RPC WITH PROTECTED UPDATES AND RETURN customer_id
...
-- 2. RECREATE clientes VIEW TO SUPPORT ALL NEW COLUMNS WITH ENGLISH AND PORTUGUESE ALIASES
DO $$
DECLARE
  v_rec RECORD;
BEGIN
  IF EXISTS (
    ...
    FOR v_rec IN SELECT * FROM public.clientes LOOP
      BEGIN
        INSERT INTO public.customers (
          id,
          tenant_id,
          ...
          v_rec.id,
          v_rec.company_id,
          COALESCE(v_rec.name, 'Cliente'),
          COALESCE(v_rec.name, 'Cliente'),
          regexp_replace(v_rec.phone, '\D', '', 'g'),
          v_rec.email,
          v_rec.created_at,
...
-- 3. RECREATE pedidos VIEW TO ALIGN FULLY WITH ALL NEW COLUMNS AND ALIASES
