...
const statusConfig: Record<OrderStatus, { label: string, color: string, icon: any, bg: string }> = {
  orcamento: { label: "Orçamento", color: "text-slate-500", icon: FileText, bg: "bg-slate-100" },
  confirmado: { label: "Confirmado", color: "text-blue-600", icon: CheckCircle2, bg: "bg-blue-50" },
  producao: { label: "Produção", color: "text-amber-600", icon: Flame, bg: "bg-amber-50" },
  finalizado: { label: "Finalizado", color: "text-primary", icon: CheckCircle2, bg: "bg-rose-50" },
  entregue: { label: "Entregue", color: "text-green-600", icon: Truck, bg: "bg-green-50" },
}

// Removendo mock antigo para evitar erros de tipo
          ...
          {filtered.map((order, i) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn("absolute top-0 left-0 right-0 h-2 opacity-80", config.bg.replace('bg-', 'bg-').replace('50', '400'))} />

                <div className="flex items-start justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <div className={cn("flex size-14 items-center justify-center rounded-2xl text-white shadow-xl shadow-primary/20 transform rotate-[-3deg] group-hover:rotate-0 transition-transform duration-500", config.bg.replace('bg-', 'bg-').replace('50', '400'))}>
