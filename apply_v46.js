...
async function applyMigration() {
  const sqlPath = path.join(__dirname, 'migration_v46_geocoding_tracking.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');
  console.log("Attempting to apply Migration V46...");
  ...
  if (error) {
      ...
      console.log("Copie o conteúdo do arquivo 'migration_v46_geocoding_tracking.sql' e");
      console.log("execute no editor SQL do Supabase (Supabase Dashboard > SQL Editor).");
      console.log("======================================================================\n");
      // Exit 0 so build pipeline or agent continues safely even if manual execution is needed.
      process.exit(0);
  } else {
      console.log("✅ Migration V46 applied successfully via RPC!");
  }
}
